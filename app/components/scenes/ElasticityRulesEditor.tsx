...
export const ElasticityRulesEditor: React.FC<ElasticityRulesEditorProps> = ({
  ...
  const stretchableScrubbers = scrubbers.filter((s) => s.mediaType === "video" || s.mediaType === "audio");
  ...
  const toggleStrategy = (scrubberId: string) => {
    ...
    const existingRuleIndex = elasticityRules.findIndex((r) => r.scrubberId === scrubberId);

    let newRules: ElasticityRule[];
    if (existingRuleIndex >= 0) {
      // Update existing rule
      newRules = elasticityRules.map((r, i) => (i === existingRuleIndex ? { ...r, strategy: newStrategy } : r));
    } else {
      // Add new rule
            ...
            <Card key={scrubber.id} className="p-3">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">{scrubber.name || "Unnamed"}</p>
                  <p className="text-xs text-muted-foreground capitalize">{scrubber.mediaType}</p>
                </div>
                <Button
                  size="sm"
