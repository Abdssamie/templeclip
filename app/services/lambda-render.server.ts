...
function validateRenderInput(input: RenderInput): void {
	if (input.durationInFrames <= 0) {
		throw new Error(
			`Invalid durationInFrames: ${input.durationInFrames}. Must be a positive number.`
		);
	}

	if (input.compositionWidth <= 0) {
		throw new Error(
			`Invalid compositionWidth: ${input.compositionWidth}. Must be a positive number.`
		);
	}

	if (input.compositionHeight <= 0) {
		throw new Error(
			`Invalid compositionHeight: ${input.compositionHeight}. Must be a positive number.`
		);
	}
